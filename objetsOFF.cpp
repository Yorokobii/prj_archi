...
void Objet::charge_OFFUV(const char nom_fichier[40])
{
	unsigned int i;
	FILE * fichier;
	unsigned int n;
	...
	{
	    fscanf(fichier,"%lf %lf %lf %lf %lf\n",&lpoints[i].x,&lpoints[i].y,&lpoints[i].z,&lpoints[i].s,&lpoints[i].t);
		...
		if (min.z>lpoints[i].z)
			min.z=lpoints[i].z;
		if (min.s>lpoints[i].s)
			min.s=lpoints[i].s;
		if (min.t>lpoints[i].t)
			min.t=lpoints[i].t;
		...
		if (max.z<lpoints[i].z)
			max.z=lpoints[i].z;
		if (max.s<lpoints[i].s)
			max.s=lpoints[i].s;
		if (max.t<lpoints[i].t)
			max.t=lpoints[i].t;
	}
...
void Objet::affiche()
{
	...
	fprintf(stderr,"\n SOMMETS\n");
		for (i=0;i<nbsommets;i++)
			fprintf(stderr,"Sommets %u [%.2f %.2f %.2f %.2f %.2f]\n",i,lpoints[i].x,lpoints[i].y,lpoints[i].z,lpoints[i].s,lpoints[i].t);
